...
def train(args, use_modal, local_rank):
    ...
    checkpoint = None
    should_load_phase1 = args.resume or args.load_ckpt_name not in (None, "", "None", "best.pt")
    ckpt_path = resolve_load_checkpoint(args, "phase1") if should_load_phase1 else None
    ...
    if checkpoint is not None and "scaler_state" in checkpoint:
    ...
    for ep in range(args.epoch):
        ...
        if not need_break and now_epoch % 1 == 0:
            ...
            if dist.get_rank() == 0:
                ...
                if need_save:
                    best_path = save_checkpoint(now_epoch, model, model_dir, optimizer, scaler, Log_file.info, args.save_best_name)
                append_metrics(args, {"phase": "phase1", "epoch": now_epoch, "train_loss": train_loss,
    ...
    _, best_path = load_best_checkpoint_for_eval(args, "phase1", model, local_rank, Log_file.info)
    item_embeddings = get_id_embeddings(model, item_word_embs, args.eval_batch_size, args, use_modal, local_rank)
    test_metrics = eval_model_amazon(10,model,users_history_for_test, users_test, item_embeddings, args.eval_batch_size, args,
...
def run_eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, best_ndcg, early_stop_count,
    ...
    valid_metrics = eval_model_amazon(10,model, user_history, users_eval, item_embeddings, batch_size, args,
...
if __name__ == "__main__":
    ...
    model_dir, output_dir = prepare_run_dirs(args, "phase1")
