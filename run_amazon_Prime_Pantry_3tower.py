...
def train(args, use_modal, local_rank):
    ...
    train_dataset = BuildTrainDataset_new_amazon_pantry(u2seq=users_train, item_content=item_word_embs, item_num=item_num,
                                      max_seq_len=args.max_seq_len, use_modal=use_modal, args=args, bins=bins)
    ...
    checkpoint = None
    ckpt_path = resolve_load_checkpoint(args, "phase3")
    ...
    for ep in range(args.epoch):
        ...
        if not need_break and now_epoch % 1 == 0:
            ...
            if dist.get_rank() == 0:
                ...
                append_metrics(args, {"phase": "phase3", "epoch": now_epoch, "train_loss": train_loss,
    ...
    _, best_path = load_best_checkpoint_for_eval(args, "phase3", model, local_rank, Log_file.info)
    item_embeddings3, item_embeddings, id_embs = get_item_embeddings_llm_3tower(model, item_word_embs, args.eval_batch_size, args, use_modal, local_rank)
    test_metrics = eval_model_2_3tower_amazon_pantry(10, model, users_history_for_test, users_test, item_embeddings3, item_embeddings, id_embs, args.eval_batch_size, args,
...
def run_eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, best_ndcg, early_stop_count,
    ...
    valid_metrics = eval_model_2_3tower_amazon_pantry(10,model, user_history, users_eval, item_embeddings3, item_embeddings, id_embs, batch_size, args, item_num, Log_file, mode, local_rank, return_full_metrics=True)
...
if __name__ == "__main__":
    ...
    model_dir, output_dir = prepare_run_dirs(args, "phase3")
